...
## ⚠️ CRITICAL: MANDATORY FIRST STEP - Retrieve Existing Visualforce Pages

**BEFORE DOING ANYTHING ELSE, YOU MUST RETRIEVE EXISTING VISUALFORCE PAGES FROM THE ORG**

This is **NOT OPTIONAL**. Every time you start a Visualforce-related task, you MUST:

### Retrieve All Visualforce Pages (MANDATORY)
...
This retrieves all existing Visualforce pages from the org to `force-app/main/default/pages/`

### Retrieve Specific Apex Controller (ONLY IF NEEDED)

If you need to check or modify a specific Apex controller used by a Visualforce page, retrieve only that specific controller:

```
<retrieve_sf_metadata>
<metadata_type>ApexClass</metadata_type>
<metadata_name>ControllerClassName</metadata_name>
</retrieve_sf_metadata>
```

Replace `ControllerClassName` with the actual controller class name (e.g., "ContactController", "AccountService")

### Why This Is Critical:

✅ **ALWAYS retrieve at the start** of any Visualforce task to understand existing pages
✅ **ALWAYS retrieve before creating** a new page to check if a similar page already exists
✅ **ALWAYS retrieve before modifying** a page to get the latest version
...
❌ **NEVER create a Visualforce page file directly without first retrieving all existing pages**
❌ **NEVER assume a page doesn't exist** - always retrieve and check first
❌ **NEVER modify a page without retrieving** the latest version first
❌ **NEVER skip the VF page retrieval** - it is MANDATORY for every Visualforce task

### Directory Location:

**Visualforce pages are stored in:** `force-app/main/default/pages/`

- Each Visualforce page has two files:
    - `PageName.page` (the Visualforce page file)
    - `PageName.page-meta.xml` (the metadata XML file)

**Apex controllers are stored in:** `force-app/main/default/classes/`
...
## Check Existing Visualforce Pages

- Before creating a new Visualforce page, check if the page already exists:
    - First, check locally in the pages directory (`force-app/main/default/pages/`)
    - Also use the `<retrieve_sf_metadata>` tool with metadata_type "ApexPage" and metadata_name "<PageName>" to check if the page exists in the Salesforce org

### Retrieve All Existing Pages

To get all existing Visualforce pages from the org:
...
To retrieve a specific Visualforce page from the org:

```
<retrieve_sf_metadata>
<metadata_type>ApexPage</metadata_type>
<metadata_name>PageName</metadata_name>
</retrieve_sf_metadata>
```

Replace `PageName` with the actual page API name (e.g., "ContactForm", "AccountSummary")

- If the page already exists (either locally or in the org):
    - Inform the user that the page is already present.
    - Ask: "Do you want to modify this existing page or create a completely new one?"
- If the page does not exist in both locations, continue with the creation process.
