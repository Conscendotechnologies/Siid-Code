...
## ⚠️ CRITICAL: MANDATORY FIRST STEP - Retrieve Existing Aura Components

**BEFORE DOING ANYTHING ELSE, YOU MUST RETRIEVE EXISTING AURA COMPONENTS FROM THE ORG**

This is **NOT OPTIONAL**. Every time you start an Aura component-related task, you MUST:

### Retrieve All Aura Components (MANDATORY)
...
This retrieves all existing Aura components from the org to `force-app/main/default/aura/`

### Retrieve Specific Aura Component (OPTIONAL)

To retrieve a specific Aura component from the org:
...
Replace `ComponentName` with the actual component name in camelCase (e.g., "contactList", "accountCard")

### Why This Is Critical:

✅ **ALWAYS retrieve at the start** of any Aura component task to understand existing components
...
❌ **NEVER create an Aura component file directly without first retrieving all existing components**
...
❌ **NEVER skip the Aura component retrieval** - it is MANDATORY for every Aura task

### Directory Location:

**Aura components are stored in:** `force-app/main/default/aura/`

- Each Aura component has its own folder with multiple files:
    - `componentName/` (folder in camelCase)
        - `componentName.cmp` (component markup - required)
        - `componentNameController.js` (JavaScript controller - optional)
        - `componentNameHelper.js` (JavaScript helper - optional)
        - `componentName.css` (CSS styles - optional)
        - `componentName.cmp-meta.xml` (metadata configuration - required)

---

## Check Existing Aura Components

- Before creating a new Aura component, check if the component already exists:
    - First, check locally in the aura directory (`force-app/main/default/aura/`)
    - Also use the `<retrieve_sf_metadata>` tool with metadata_type "AuraDefinitionBundle" and metadata_name "<ComponentName>" to check if the component exists in the Salesforce org

### Retrieve All Existing Components

To get all existing Aura components from the org:
...
Replace `ComponentName` with the actual component name in camelCase (e.g., "contactForm", "opportunityList")

- If the component already exists (either locally or in the org):
    - Inform the user that the component is already present.
    - Ask: "Do you want to modify this existing component or create a completely new one?"
- If the component does not exist in both locations, continue with the creation process.
