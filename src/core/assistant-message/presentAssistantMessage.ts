...
import { retrieveSchemaTool } from "../tools/retrieveSchemaTool"
		...
		case "tool_use":
			const toolDescription = (): string => {
				switch (block.name) {
					...
					case "retrieve_schema":
						return `[${block.name} for '${block.params.component_name}'${block.params.schema_file ? ` in ${block.params.schema_file}` : ""}]`
				}
			}
			...
			switch (block.name) {
				...
				case "retrieve_sf_metadata":
					...
					break
				case "retrieve_schema":
					await retrieveSchemaTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag)
					break
			}
