...
function resolveMetadataPath(metadataType: string, metadataName: string, cwd: string): string | undefined {
	...
	// For bundle types (LWC, Aura), the metadata name is the directory name
	const isBundleType = metadataType === "LightningComponentBundle" || metadataType === "AuraDefinitionBundle"
...
export async function deploySfMetadataTool(
	...
	try {
		...
		if (!metadataType) {
			...
			cline.recordToolError("deploy_sf_metadata")
			pushToolResult(await cline.sayAndCreateMissingParamError("deploy_sf_metadata", "metadata_type"))
			return
		}
		...
		if (!metadataName) {
			cline.consecutiveMistakeCount++
			cline.recordToolError("deploy_sf_metadata")
			pushToolResult(await cline.sayAndCreateMissingParamError("deploy_sf_metadata", "metadata_name"))
			return
		}
